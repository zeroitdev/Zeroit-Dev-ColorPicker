...
namespace ZeroitDevColorPicker.Helpers.ColorWheel
{
  ...
  public class ZeroitColorWheel : Control, IColorEditor
  {
    #region Constants
    ...
    private Color wheelBorderColor = Color.White;

    private int wheelBorder = 3;
    ...
    public Color WheelBorderColor
    {
            get { return wheelBorderColor; }
            set
            {
        ...
        public int WheelBorder
        {
            get { return wheelBorder; }
            set
            {
                wheelBorder = value;
                Invalidate();
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
      ...
      if (this.AllowPainting)
      {
        ...
        {
          ButtonRenderer.DrawParentBackground(g, this.DisplayRectangle, this);
        }

        if (_brush != null)
        {
          g.FillPie(_brush, this.ClientRectangle, 0, 360);
        }
        //// HACK: smooth out the edge of the wheel.
        //// https://github.com/cyotek/Zeroit.Framework.ColorPicker.ColorPicker/issues/1 - the linked source doesn't do this hack yet draws with a smoother edge
        //e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
        using (Pen pen = new Pen(wheelBorderColor, wheelBorder))
        {
    ...
    protected virtual void PaintColor(PaintEventArgs e, HslColor color, bool includeFocus)
    {
      ...
      if (!float.IsNaN(location.X) && !float.IsNaN(location.Y))
      {
        ...
        if (this.SelectionGlyph == null)
        {
          g.DrawRectangle(Pens.Black, x, y, this.SelectionSize, this.SelectionSize);
        }
        else
        {
          g.DrawImage(this.SelectionGlyph, x, y);
        }

        if (this.Focused && includeFocus)
        {
          ControlPaint.DrawFocusRectangle(g, new Rectangle(x - 1, y - 1, this.SelectionSize + 2, this.SelectionSize + 2));
        }
      }
    }
